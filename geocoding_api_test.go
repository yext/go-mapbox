...
func TestQueryByAddress(t *testing.T) {
	...
	if len(res.Features) != 1 {
		t.Fatalf("expected 1 feature, got: %v", len(res.Features))
	}
	f := res.Features[0]
	if len(f.Center) != 2 {
		t.Errorf("expected center with 2 coordinates, got: %v", len(f.Center))
	} else {
		if f.Center[0] != 11.54165 {
			t.Errorf("expected center longitude of 11.54165, got: %v", f.Center[0])
		}
		if f.Center[1] != 48.151313 {
			t.Errorf("expected center latitude of 48.151313 , got: %v", f.Center[1])
		}
	}
	...
	if f.ID != "city.10676757" {
		t.Errorf("expected id %q, got: %v", "city.10676757", f.PlaceName)
	}
